...
export class BoardComponent implements OnInit {
  ...
  constructor(private service: GameService, private storage: StorageService, private mongo: ConnectionsService, private pets: PetService) {}
  ...
  initNext() {
    this.mongo.getData(this.storage.email,this.storage.userToken,"THighscore").then((val) =>{this.highScore=JSON.parse(val)})
  ...
  gameOver() {
    ...
    cancelAnimationFrame(this.requestId);
    this.mongo.getData(this.storage.email,this.storage.userToken,"THighscore").then((val) =>
    {
      if (this.points > JSON.parse(val)){
        this.highScore = this.points
        this.mongo.setData(this.storage.email,this.storage.userToken,"THighscore", JSON.stringify(this.highScore)).then(()=>{})
      }


    })
    if(this.points > 1000){
      var value = Math.floor(this.points/1000)
      this.mongo.getData(this.storage.email,this.storage.userToken,"funds").then((val) =>{
      var addedVal = JSON.parse(val) + value
      this.mongo.setData(this.storage.email,this.storage.userToken,"funds", JSON.stringify(addedVal)).then((val2) => {})
      this.pets.increaseHappiness(Math.floor(addedVal/2));
      }
      )
